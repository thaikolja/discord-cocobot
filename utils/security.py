...
class InputValidator:
    ...
    def validate_discord_id(
        cls, discord_id: str, field_name: str = "discord_id"
    ) -> str:
        """
        ...
        if not cls.DISCORD_ID_PATTERN.match(discord_id):
            raise ValidationError(
                f"Invalid {field_name} format. Must be 17-20 digits", field_name
            )
    ...
    def validate_currency_code(cls, currency: str, field_name: str = "currency") -> str:
        ...
        if not cls.CURRENCY_CODE_PATTERN.match(currency):
            raise ValidationError(
                f"Invalid {field_name} format. Must be 3 uppercase letters", field_name
            )
    ...
    def validate_length(
        ...
        if max_length is not None and len(value) > max_length:
            raise ValidationError(
                f"{field_name} must be no more than {max_length} characters", field_name
            )
    ...
    @classmethod
    def validate_choice(
        cls, value: str, choices: List[str], field_name: str = "value"
    ) -> str:
        """
    ...
    def check_command_injection_patterns(cls, content: str) -> bool:
        ...
        for pattern in cmd_patterns:
            if re.search(pattern, content):
                ...
                if re.search(
                    r'\b(echo|cat|ls|pwd|whoami|rm|mv|cp|chmod|chown|ps|kill|grep|find|touch|mkdir|rmdir|cd|env|export|unset|source|eval|exec|command)\b',
                    content.lower(),
                ):
                    return True
...
def validate_and_sanitize_input(
    input_value: str, input_type: str = "text", **kwargs
) -> str:
    """
...
def safe_format_string(template: str, **kwargs) -> str:
    ...
    for placeholder in placeholders:
        ...
        if (
            not re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*$', clean_placeholder)
            and clean_placeholder != ''
        ):
            raise SecurityError(
                "Template contains potentially dangerous format patterns"
            )
