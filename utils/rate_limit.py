...
from .database import get_db_session, DatabaseManager, RateLimit
...
class DatabaseRateLimiter:
    ...
    def is_allowed(self, identifier: str, resource: str, max_requests: int, window_seconds: int) -> Tuple[bool, float]:
        """
        ...
        with get_db_session() as db:
            ...
            if datetime.utcnow() > rate_limit.reset_at:
                ...
                rate_limit.reset_at = datetime.utcnow() + timedelta(seconds=window_seconds)
                db.commit()
                return True, 0.0
            ...
            if rate_limit.requests_count >= max_requests:
                ...
                time_since_reset = datetime.utcnow() - (rate_limit.reset_at - timedelta(seconds=window_seconds))
                elapsed = time_since_reset.total_seconds()
                retry_after = max(0, window_seconds - elapsed)
                
                self.logger.warning(f"Rate limit exceeded for {identifier} on resource {resource}")
    ...
    def is_allowed(self, identifier: str, resource: str, max_requests: int, window_seconds: int) -> Tuple[bool, float]:
        ...
        with self._lock:
            ...
            self.limits[key]['requests'] = [
                req_time for req_time in self.limits[key]['requests'] 
...
class HybridRateLimiter:
    ...
    def is_allowed(self, identifier: str, resource: str, max_requests: int, window_seconds: int, 
                   use_db: bool = True) -> Tuple[bool, float]:
        """
...
class CommandRateLimiter:
    ...
    def check_command_limit(self, user_id: str, command_name: str, max_requests: Optional[int] = None,
                           use_db: bool = True) -> Tuple[bool, float]:
        """
    ...
    def check_global_command_limit(self, max_requests: int = 100, use_db: bool = True) -> Tuple[bool, float]:
        """
